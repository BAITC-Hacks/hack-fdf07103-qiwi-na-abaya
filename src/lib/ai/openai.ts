...
Для evidence.title скопируй rawDescription ДОСЛОВНО целиком. Например: evidence.title = «Мы теряем клиентов интернет-магазина и хотим понять почему.», card.title = «Причины потери клиентов интернет-магазина». Никогда не записывай название из card.title в evidence.title. Для остальных полей card копируй соответствующий evidence без добавления фактов. Ответ используй только для того раздела, к которому относится его key. Не выводи отрасль из предположений: можно скопировать прямо названный «интернет-магазин».
