...
export function calculateReadiness(fields: ScoringFields) {
  ...
  const definitions: { label: string; fields: (keyof ScoringFields)[]; max: number; hint: string }[] = [
    { label: "Контекст и потребность", fields: ["context", "need"], max: 20, hint: "Опишите ситуацию и проблему, которую нужно решить." },
    { label: "Данные и материалы", fields: ["dataMaterials"], max: 20, hint: "Укажите доступные данные и способ доступа к ним." },
    { label: "Ожидаемый результат", fields: ["expectedResult"], max: 15, hint: "Назовите конкретный результат работы команды." },
    { label: "Критерии успеха", fields: ["successCriteria"], max: 15, hint: "Опишите, как бизнес проверит результат." },
    { label: "Ограничения", fields: ["constraints"], max: 10, hint: "Укажите сроки, бюджет и технические ограничения." },
    { label: "Пользователи", fields: ["users"], max: 10, hint: "Укажите, кто будет пользоваться решением." },
    { label: "Связь с бизнесом", fields: ["businessContact"], max: 10, hint: "Укажите контакт и формат взаимодействия." },
  ];
